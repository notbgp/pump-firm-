...
export default function TradingTerminal() {
  ...
  const [chartCoin, setChartCoin] = useState<PumpCoin | null>(null);
  ...
  const executeSwap = async () => {
    if (!publicKey || !selectedCoin) return;
    ...
    try {
      ...
      if (!selectedCoin.complete && tradeMode === 'buy' && quote?.isPumpFun) {
        ...
        const response = await fetch('/api/pumpfun-buy', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            tokenMint: selectedCoin.mint,
        ...
        setStatus(`✅ Bought on Pump.fun! TX: ${txid.slice(0, 8)}...`);
        setQuote(null);
        setAmount('');
      ...
      // JUPITER DEX - For graduated tokens or sells
        ...
        setStatus('Executing swap on Jupiter...');
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      {chartCoin && (
        ...
        >
          <div 
            className="bg-[#111] rounded-xl border border-gray-800 w-full max-w-6xl h-[80vh] flex flex-col overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 border-b border-gray-800 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                <img src={chartCoin.image_uri} alt={chartCoin.name} className="w-10 h-10 rounded-lg" />
                <div>
                  <h2 className="text-xl font-bold">{chartCoin.symbol}</h2>
                  <p className="text-sm text-gray-400">{chartCoin.name}</p>
                </div>
                <div className="ml-4">
                  <div className="text-sm font-bold text-green-400">{formatMarketCap(chartCoin.market_cap)}</div>
                  <div className="text-xs text-gray-500">Market Cap</div>
                </div>
                {!chartCoin.complete && (
                  <div className="px-2 py-1 bg-purple-600/20 border border-purple-500/30 rounded text-xs font-bold text-purple-400">
                    🔥 PUMP.FUN
                  </div>
                )}
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    setSelectedCoin(chartCoin);
                    setChartCoin(null);
                  }}
                  className="px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 rounded-lg text-sm font-bold"
                >
                  Trade {chartCoin.symbol}
                </button>
                <button
                  onClick={() => setChartCoin(null)}
            ...
            <div className="flex-1 bg-[#0a0a0a] overflow-hidden">
              <iframe
                src={`https://dexscreener.com/solana/${chartCoin.mint}?embed=1&theme=dark&trades=0&info=0`}
                className="w-full h-full border-0"
                title={`${chartCoin.symbol} Chart`}
              />
            </div>
          </div>
        </div>
            ...
            {selectedCoin ? (
              <div className="flex-1 flex flex-col overflow-y-auto">
                ...
                <div className="flex items-center gap-2 mb-3 p-2 bg-gray-900/50 rounded-lg flex-shrink-0">
                  ...
                  {!selectedCoin.complete && (
                    <div className="px-1.5 py-0.5 bg-purple-600/20 border border-purple-500/30 rounded text-xs font-bold text-purple-400">
                      PUMP.FUN
                    </div>
                  )}
                  <button
                    onClick={() => setChartCoin(selectedCoin)}
                    className="px-2 py-1 bg-purple-600/20 text-purple-400 hover:bg-purple-600/30 rounded text-xs font-medium"
                  >
                    Chart
                  </button>
                </div>
